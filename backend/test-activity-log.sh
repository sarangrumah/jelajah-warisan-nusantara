...
# Test script for activity log endpoints
# Run this after installing dependencies and rebuilding
...
echo "---------------------------------------------------"
curl -X POST "$BASE_URL/api/activity-log" \
  -H "Content-Type: application/json" \
  -d '{
    "user_type": "guest",
    "activity_type": "test_activity",
    "details": {"test": "endpoint_verification"},
    "success": true
  }' \
...
curl -X GET "$BASE_URL/api/activity-log" \
  -w "\nHTTP Status: %{http_code}\n" \
  -s
echo ""
echo ""
